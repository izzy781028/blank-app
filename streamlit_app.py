...
dict_style = {
    ...
    "Y2K風": "Y2K Cyber Pop aesthetic, early 2000s style, bubblegum pink and neon green, low-resolution digital texture, overexposed dreamy but noisy digital texture, nostalgic cyber party mood, high saturation, high contrast, nostalgic, ultra detailed, highres, ultra-detailed", 
    ...
    "3D 視覺風": "3D animation style, high-end CGI, smooth 3D render, rounded simplified body shape, tiny soft limbs, adorable exaggerated expression, 3D soft sculpture, pastel candy color palette, high-key soft lighting, subsurface scattering, low contrast, clean bright composition, gentle shadows, kawaii dreamcore, cozy and playful atmosphere, cinematic 3D lighting, ultra detailed, high resolution"
}
...
mode = st.radio(
    "模式選擇",["一般生成模式 (從零開始)", "畫面重構模式 (換狀態/換視角)", "分鏡保留模式 (換主體/換場景)", "角色設計模式 (三視圖/設定集)"],
    horizontal=True,
    label_visibility="collapsed"
)

is_remake_mode = mode == "畫面重構模式 (換狀態/換視角)"
is_layout_mode = mode == "分鏡保留模式 (換主體/換場景)"
...
if is_normal_mode:
    ...
    st.info("**畫面重構模式**：繼承參考圖 [Image 1] 的主體外貌與光影。您可以修改狀態、場景，並強制更改鏡頭視角。")
elif is_layout_mode:
    st.info("**分鏡保留模式**：鎖定參考圖 [Image 1] 的所有攝影機位置與構圖。您可以將畫面的主體、狀態、背景或光線換掉。")
elif is_character_mode:
    st.info("**角色設計模式**：為指定主角生成包含「全身多角度」與「臉部特寫」的專業 8 格角色設定圖。")

# --- 【第一區：核心內容】 ---
st.markdown("<div class='section-header'>畫面核心內容 (Subject, Action/State, Where)</div>", unsafe_allow_html=True)
col_text1, col_text2, col_text3 = st.columns(3)

with col_text1:
    if is_remake_mode:
        subj_label = "畫面主體 (Subject) 🔒 [重構模式已鎖定]"
        subj_val = "同主參考圖主體"
        subj_disabled = True
    else:
        subj_label = "畫面主體 (Subject - 人/物)"
        subj_val = "" if is_layout_mode else "一位穿白襯衫的年輕台灣女性"
        subj_disabled = False
        
    subj_help = "若留白，將維持參考圖原本的主體" if is_layout_mode else "*必填 (可輸入人物或物品，如一瓶紅色香水)"
    user_keyword = st.text_input(subj_label, value=subj_val, disabled=subj_disabled, help=subj_help, placeholder="例如: 一位台灣男性 或 一個紅色皮革包")
...
with col_text2:
    if is_character_mode:
        ...
        act_help = "若留白，將維持原狀態與姿勢" if (is_remake_mode or is_layout_mode) else "(可定義面向、表情或物品擺放狀態) 非必填"
        act_val = "" if (is_remake_mode or is_layout_mode) else "看向窗外，手裡拿著咖啡，神情放鬆"
        
    user_action = st.text_input(
        "主體狀態/動作 (Action/State)", 
        value=act_val, 
        placeholder="例如：看向窗外 / 放置在大理石桌面上", 
        help=act_help
    )
...
with col_text3:
    ...
    if is_remake_mode or is_layout_mode:
        ...
        bg_help = "可自訂場景。若留白且「未使用」場景參考圖，預設為純白背景"
        bg_value = ""
    else:
        bg_help = "*必填"
        bg_value = "陽光明媚的現代咖啡廳"
...
st.info(
    ...
    "1. **高畫質且清晰**，主體佔比明確。\n"
    "2. **背景不要太複雜**，乾淨的背景能讓 AI 更容易抓取主體與物件。\n"
...
with col_ref4:
    if is_remake_mode:
        ...
        st.checkbox("光線與色調 🔒 [角色模式已鎖定]", value=False, disabled=True, help="角色設定圖將強制使用預設的寫實光影設定。")
    else:
...
is_top_down = ("Top-Down" in angle_choice)
    ...
    if (is_normal_mode or is_character_mode) and user_keyword.strip() == "":
        ...
        if is_remake_mode:
            ...
            char_sheet_template = (
                ...
                f"{bg_str}"
                "The style must be photorealistic, shot on DSLR, 35mm lens, muted color tones, delicate midtone detail, subtle film grain, refined texture clarity"
            )
