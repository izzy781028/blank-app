...
with col_text2:
    user_action = st.text_input(
        "🏃‍♂️ 主角動作 (Doing What)", 
        value="" if is_remake_mode else "直視鏡頭，手裡拿著咖啡，燦爛微笑", 
        placeholder="例如：直視鏡頭、看向遠方、手裡拿著咖啡...", 
...
if "Back View" in relation_choice and any(word in user_action for word in face_keywords):
    conflicts.append("👤 **面向衝突**：選擇了「背後視角」，但動作中包含了「臉部表情/視線」。AI 預設背影無法看到臉，可能會畫出詭異扭曲的畫面，或直接忽略指令。")

# 3. 極特寫 + 過肩鏡頭衝突
if shot_choice == "極特寫" and "Over the shoulder" in relation_choice:
    conflicts.append("📷 **鏡頭衝突**：「極特寫」視野極小（通常只拍眼睛或嘴唇），無法容納「過肩鏡頭」所需要的肩膀前景。建議鏡頭大小改為「半身」或「特寫」。")
